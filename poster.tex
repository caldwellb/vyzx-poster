...
\textbf{The Coq Proof Assistant} 
  ...
  The Coq proof assistant is a programming language designed to allow us to formalize and write proof for mathematical statments.
  %
  Coq uses a dependent type system which allows us to write types which correspond to logical statements.
  %
  We can then supply a value of the specified type, which will serve as proof of the statement.
  %
  In this way Coq allows us to create constructive proof.
  %
  This proof system is not just used to prove mathematical statements.
  %
  The verified quantum circuit optimizer \VOQC uses the Coq proof assistant to verify that the optimizations it performs do not change the circuit semantics.
  ...
  \end{wrapfigure}
    ...
    The ZX calculus is an intermediate representation for quantum programs.
    %
    The core objects of the ZX calculus are the Z and X spiders in \Cref{fig:Zspider,fig:Xspider}, which are abstractions of both Z and X rotations.
    %
    ...
    %
    Programs like PyZX\cite{kissinger2020Pyzx} take advantage of these rules in order to create quantum circuit optimizers.
...
\begin{column}{\colwidth}
  ...
  \begin{alertblock}{Inductively Drawing Diagrams}
    ...
    \begin{figure}
      \begin{tabular}{| c | c | c | c | c | c | c | c |}
        \hline
        Constructor & Empty & Cap & Cup & Swap & Compose $D_1$ $D_2$ & Stack $D_1$ $D_2$ & Wire \\
        \hline
  ...
  \begin{block}{Diagram Semantics}
    ...
    \begin{figure}
      ...
      \caption{The diagram semantics asignment function for inductive ZX calculus}
    \end{figure}

    Using these semantics we define a relation $\propto$ given by
    \[
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Example: Untangling Nots}
    ...
    \(
      \texttt{Definition CNOT\_R} := (\Z{1}{2}{0} \vyzxstack \vyzxwire) \vyzxcompose (\vyzxwire \vyzxstack \X{2}{1}{0})
    \)
    \begin{figure}
      \tikzfig{cnot-r}
    \end{figure}
    ...
    \(
      \texttt{Definition CNOT\_L} := (\vyzxwire \vyzxstack \X{2}{1}{0}) \vyzxcompose (\Z{1}{2}{0} \vyzxstack \vyzxwire)
    \)
    \begin{figure}[h]
      \tikzfig{cnot-l}
    \end{figure}
    ...
    \[
     ...
     \]
     Using the proof automation available to \QLib, this equality can be solved automatically.
     %
     We can also check that we have the correct construction using the same methods to prove that our semantics match the \texttt{cnot} gate from \QLib.

    \(
    \texttt{Lemma CNOT\_CORRECT : }\texttt{semantics}(CNOT_R) = \texttt{cnot}
    \)
